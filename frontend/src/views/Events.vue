<template>
    <section class="mx-auto max-w-7xl px-5 py-16">
        ...
        <div v-else class="mt-10 grid gap-6 md:grid-cols-3">
            <article v-for="e in events" :key="e._id" class="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-200">
                <img v-if="e.image" :src="e.image" :alt="e.title" class="h-48 w-full object-cover">
                <div class="p-6">
